          ...
          <div className="grid grid-cols-2 gap-6">
            {weatherDetails.map((detail, index) => (
              <motion.div
                ...
                className="bg-white/5 rounded-2xl p-6 border border-white/10 flex flex-col items-center justify-center"
              >
                <detail.icon className="w-10 h-10 text-blue-400 mb-4" />
                <h4 className="text-lg font-semibold text-white mb-2">{detail.label}</h4>
                <p className="text-2xl font-bold text-blue-300">{detail.value}</p>
              </motion.div>
            ))}
          </div>
        </div>
